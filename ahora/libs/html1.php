...
function do_header($title, $id='home', $options = false) {
	...
	$sites = $db->get_results("select * from subs where visible order by orden asc");
	...
	if (! is_array($options)) {
		...
		$right_options[] = new MenuOption(_('fisgonaaaaaaaa'), $globals['base_url'].'sneak', $id, _('visualizador en tiempo real'));
