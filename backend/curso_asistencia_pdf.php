...
/** VARIABLE **/
$registros = 38; // registros que caben en una página
/**/

$modalidadcurso = $_GET['modalidad'];
$tipomodalidad = $_GET['tipomodalidad'];

if ($modalidadcurso==2){
	$sql="SELECT * FROM curso_usuario AS cu, usuario AS u WHERE cu.inscripciononlinepresencial='$tipomodalidad' AND cu.idusuario=u.id AND cu.nivel='5'  AND cu.estado=0 AND cu.espera=0 AND cu.pagado!=0  AND cu.idcurso='$idcurso' AND cu.borrado=0 ORDER BY cu.inscripciononlinepresencial,u.apellidos, u.nombre, cu.id DESC ";
}
else{
	$sql="SELECT * FROM curso_usuario AS cu, usuario AS u WHERE cu.idusuario=u.id AND cu.nivel='5'  AND cu.estado=0 AND cu.espera=0 AND cu.pagado!=0  AND cu.idcurso='$idcurso' AND cu.borrado=0 ORDER BY cu.inscripciononlinepresencial,u.apellidos, u.nombre, cu.id DESC ";
}
...
$resultc=pg_query($link,$sql) ;//or die (pg_error());  
...
for ($i=1; $i<=$paginas; $i++){
	...
	if ($modalidadcurso==2){
		$sql="SELECT * FROM curso_usuario AS cu, usuario AS u WHERE cu.inscripciononlinepresencial='$tipomodalidad' AND cu.idusuario=u.id AND cu.nivel='5'  AND cu.estado=0 AND cu.espera=0 AND cu.pagado!=0  AND cu.idcurso='$idcurso' AND cu.borrado=0 ORDER BY cu.inscripciononlinepresencial,u.apellidos, u.nombre, cu.id DESC LIMIT $registros OFFSET $inicio; ";
	}
	else{
		$sql="SELECT * FROM curso_usuario AS cu, usuario AS u WHERE cu.idusuario=u.id AND cu.nivel='5'  AND cu.estado=0 AND cu.espera=0 AND cu.pagado!=0  AND cu.idcurso='$idcurso' AND cu.borrado=0 ORDER BY cu.inscripciononlinepresencial,u.apellidos, u.nombre, cu.id DESC LIMIT $registros OFFSET $inicio;";
	}
	...
	while(($row = pg_fetch_array($result))) { 
		...
		$resulta=posgre_query("SELECT * FROM curso_horario_asistencia WHERE idcursohorario IN (SELECT id FROM curso_horario WHERE idcurso='$idcurso' AND borrado=0) AND estado=0 AND idcursousuario='$idcursousuario' AND borrado=0;") ;//or die ("ERROR AL MODIFICAR. ".mysql_error()); 
		...
		$asistencia = number_format($cuantos, 2, '.', '');
		...
		<td class="cabecera">
				...
				<tr>
					...
					<td class="logocolegios">
						<p>
							Listado de asistencias. P&aacute;gina <?=$i?>/<?=$paginas?><br/>
						</p>
					</td>
				</tr>
				</table>
...
try
{
	...
	$html2pdf->Output('asistencias'.$idcurso.'.pdf');
}
