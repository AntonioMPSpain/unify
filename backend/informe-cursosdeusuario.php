...
$link=iConectarse();
$result=pg_query($link,"SELECT * FROM curso_usuario WHERE nivel<>'3' AND idusuario='$idusuario' AND borrado=0 ORDER BY  $sqlcampo $sqlorden, id DESC") ;//or die (pg_error());  
	...
	while(($row = pg_fetch_array($result))) { 
		...
		if ($total>0){
			...
			<tr>
				...
				<td><?=$estadotexto?></td>
				<td><a href='zpa_usuario_curso.php?idcurso=<?=$idcurso.$getcursodual?>'><?=$modalidadtexto?><a></td>
				<td><?=$row["precio"];?></td>
				<td><?=$pagadotexto?></td>
				<td>
						<a class="btn btn-primary" href="informe-cursosdeusuario_pdf.php?idcurso=<?=$idcurso?>&idusuario=<?=$idusuario?>" title="pdf">Certificado asistencia</a>
   
					<!--<a class="btn btn-primary" href="diploma.php?idcurso=<?=$idcurso?>&idusuario=<?=$idusuario?>" title="pdf">Diploma</a>-->
				</td>
			</tr>
			<?
		}
			...
			<a class="btn btn-primary" href="informe-cursosdeusuario_pdf.php?idusuario=<?=$idusuario?>" title="pdf">Certificado cursos asistidos</a>
