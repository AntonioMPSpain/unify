	...
	<h2 class="titulonoticia">Inserta Curso</h2>
		...
		<!-- <div class="bloque-lateral acciones">		
			<p><strong>Acciones:</strong>
				<a class="btn btn-success" href="e_nueva.php">Volver <i class="icon-circle-arrow-left"></i></a> <br />
			</p>
		</div> -->
		<!--fin acciones-->
		...
		<tr>
			<th>REF.</th>
			<th>FECHA INICIO</th>
			<th><a href="e_curso.php?campo=titulo&orden=ASC">TÍTULO</a></th>
...
//fin Paginacion 1
	while($row = pg_fetch_array($result)) { 
		...
		<tr>
			...
			<td>
				<? if ($accion=="duplicar"){
					?> <a href="e_duplicar.php?idduplicar=<?=$idduplicar?>&c=<?=$row['id']?>" class="btn btn-primary">Seleccionar</a> <?
				}
				else{
					?> <a href="e_nueva.php?c=<?=$row['id']?>" class="btn btn-primary">Seleccionar</a> <?
				}
				?>
			</td>
		</tr>
    ...
    <p class="align-center">Total: <?=$total_registros?> cursos</p>
			...
			if($total_paginas > 1) { ?>
				<div class="pagination">
					<ul>
						<li<? if ($pagina==1){?> class="disabled"<? }?>><a href="e_curso.php?pagina=<?=(1)?>&texto=<?=$texto?>" title="Ver primeros Resultados">Primeros</a></li><?
						if(($pagina - 1) > 0) { ?>
							<li><a href="e_curso.php?accion=<?=$accion?>&idduplicar=<?=$idduplicar?>&pagina=<?=($pagina-1)?>&texto=<?=$texto?>" title="Ver anteriores Resultados">Anteriores</a></li>
							<?
						}
						$a=$pagina-2;
						$b=$pagina+2;
						for ($i=1; $i<=$total_paginas; $i++){ 
							if ($pagina == $i) {
								...
								if (($a<$i)&&($b>$i)){
									?>
									<li><a href="e_curso.php?accion=<?=$accion?>&idduplicar=<?=$idduplicar?>&pagina=<?=$i?>&texto=<?=$texto?>"><?=$i?></a></li>
									<?
								}
							}	
						}
						if(($pagina + 1)<=$total_paginas) { ?>
							 <li><a href="e_curso.php?accion=<?=$accion?>&idduplicar=<?=$idduplicar?>&pagina=<?=($pagina+1)?>&texto=<?=$texto?>" title="Ver siguientes Resultados">Siguientes</a></li>
							<?
						} ?>
						<li<? if ($pagina==$total_paginas){?> class="disabled"<? }?>><a href="e_curso.php?accion=<?=$accion?>&idduplicar=<?=$idduplicar?>&pagina=<?=$total_paginas?>&texto=<?=$texto?>" title="Ver &uacute;ltimos resultados">&Uacute;ltimos</a></li>
					</ul>
				</div>
				<?
			}?>
